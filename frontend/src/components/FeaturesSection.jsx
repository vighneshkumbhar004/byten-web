...
const FeatureItem = ({ feature, index }) => {
  ...
  return (
    <motion.div
      ref={ref}
      style={{ opacity }}
      className={`flex flex-col ${feature.imagePosition === "left" ? "lg:flex-row" : "lg:flex-row-reverse"} gap-16 items-center mb-40 last:mb-0 group`}
    >
      ...
      >
        <div className="relative rounded-2xl overflow-hidden shadow-2xl">
          <motion.div
            ...
            animate={{ background: isHovered ? "linear-gradient(45deg, #FFCC00, transparent, #FFCC00)" : "linear-gradient(45deg, transparent, transparent, transparent)" }}
            style={{ padding: "3px", WebkitMask: "linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)", WebkitMaskComposite: "xor", maskComposite: "exclude" }}
          />
          <motion.div className="relative h-[500px] overflow-hidden rounded-2xl" style={{ y: imageY }}>
            {feature.video ? (
              <video
                src={feature.video}
                autoPlay
            ...
            <motion.div
              ...
              animate={{ background: isHovered ? "linear-gradient(135deg, rgba(255,204,0,0.3) 0%, rgba(10,17,26,0.6) 100%)" : "linear-gradient(135deg, rgba(255,204,0,0.1) 0%, rgba(10,17,26,0.3) 100%)" }}
      ...
      >
        ...
        >
          ...
          <p className="text-gray-600 italic mb-6 text-lg leading-relaxed relative z-10 font-sans">"{feature.quote.text}"</p>
          <div className="flex items-center justify-between relative z-10 mb-6">
            <div>
              <p className="font-bold text-gray-900 text-lg font-heading">{feature.quote.author}</p>
              <p className="text-sm text-gray-500 mt-1 font-sans">{feature.quote.company}</p>
            </div>
          </div>
          <button
